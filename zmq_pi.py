...
B=32 # number of bits of precision
...
def bitsource(zcontext,url):
	...
	zsock.bind(url)
	while True:
		zsock.send_string(ones_and_zeros(B*2)) # generate a binary string with 2B length
		time.sleep(0.01)
...
def judge(zcontext, in_url,pythagoras_url,out_url):
	...
	while True:
		...
		osock.send_string('Y' if sumsquares < unit else 'N') # shorthand for inline if else 
...
def start_thread(function,*args):
	...
	thread.daemon=True # can terminate by sending sigint
	thread.start()
